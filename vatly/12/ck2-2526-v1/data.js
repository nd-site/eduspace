// data.js – KIỂM TRA CUỐI KÌ II – VẬT LÝ 12 (KNTT) – 2025-2026
// Chương trình GDPT 2018 | Sách Kết Nối Tri Thức với Cuộc Sống
// Nội dung: Dao động điện – Sóng điện từ – Quang học – Lượng tử – Hạt nhân
// 28 câu TN (×0,25đ = 7đ) + 3 câu TL (×1đ = 3đ) | 50 phút

window.quizData = {
    title: "Kiểm tra Cuối Học Kì II – Vật Lý 12 (Kết nối tri thức) 2025-2026",
    config: {
        testDuration: 3000,
        examLayout: {
            multiple: { count: 28, pointsPerQ: 0.25 },
            essay: { count: 3, pointsPerQ: 1.0 }
        }
    },
    questions: [
        // ── DAO ĐỘNG ĐIỆN TỪ ──────────────────────────────────────────────
        {
            type: "multiple",
            question: "Trong mạch dao động LC lý tưởng, khi năng lượng điện trường đạt cực đại thì năng lượng từ trường:",
            options: ["Cũng đạt cực đại", "Bằng 0", "Bằng một nửa cực đại", "Không xác định"],
            correct: 1,
            explanation: "Cơ chế chuyển hóa: khi Wđiện cực đại (tụ tích điện cực đại) thì dòng qua L = 0 → Wtừ = 0."
        },
        {
            type: "multiple",
            question: "Chu kỳ dao động riêng của mạch LC là:",
            options: [
                "$$T = 2\\pi\\sqrt{LC}$$",
                "$$T = 2\\pi\\sqrt{\\frac{L}{C}}$$",
                "$$T = \\frac{1}{2\\pi\\sqrt{LC}}$$",
                "$$T = \\frac{2\\pi}{\\sqrt{LC}}$$"
            ],
            correct: 0,
            explanation: "T = 2π√(LC); tần số f = 1/(2π√LC)."
        },
        {
            type: "multiple",
            question: "Sóng điện từ trong chân không truyền với vận tốc:",
            options: ["3×10⁸ km/s", "3×10⁸ m/s", "3×10⁶ m/s", "Tùy tần số"],
            correct: 1,
            explanation: "Vận tốc ánh sáng trong chân không c = 3×10⁸ m/s, không phụ thuộc tần số."
        },
        {
            type: "multiple",
            question: "Mạch LC có L = 0,4 mH, C = 40 pF. Tần số dao động riêng là:",
            options: ["≈ 1,26 MHz", "≈ 398 kHz", "≈ 12,6 MHz", "≈ 39,8 kHz"],
            correct: 0,
            explanation: "f = 1/(2π√LC) = 1/(2π√(0,4×10⁻³×40×10⁻¹²)) ≈ 1/(2π×4×10⁻⁷) ≈ 1,26×10⁶ Hz."
        },
        {
            type: "multiple",
            question: "Sóng điện từ nào sau đây có bước sóng dài nhất?",
            options: ["Tia X", "Ánh sáng nhìn thấy", "Tia hồng ngoại", "Sóng vô tuyến"],
            correct: 3,
            explanation: "Thứ tự tăng dần bước sóng: tia γ < tia X < UV < ánh sáng < IR < sóng vô tuyến."
        },
        // ── QUANG HÌNH HỌC ────────────────────────────────────────────────
        {
            type: "multiple",
            question: "Định luật Snell (khúc xạ ánh sáng) được biểu diễn bằng:",
            options: [
                "n₁sinθ₁ = n₂sinθ₂",
                "n₁cosθ₁ = n₂cosθ₂",
                "n₁sinθ₁ = n₂tanθ₂",
                "sinθ₁/sinθ₂ = n₁.n₂"
            ],
            correct: 0,
            explanation: "Định luật khúc xạ: n₁sinθ₁ = n₂sinθ₂."
        },
        {
            type: "multiple",
            question: "Góc giới hạn phản xạ toàn phần θ_gh của môi trường có chiết suất n₂ < n₁ được tính bởi:",
            options: [
                "$$\\sin\\theta_{gh} = n_1/n_2$$",
                "$$\\sin\\theta_{gh} = n_2/n_1$$",
                "$$\\tan\\theta_{gh} = n_2/n_1$$",
                "$$\\cos\\theta_{gh} = n_2/n_1$$"
            ],
            correct: 1,
            explanation: "sinθgh = n₂/n₁ (ánh sáng đi từ môi trường chiết quang hơn sang kém hơn)."
        },
        {
            type: "multiple",
            question: "Thấu kính hội tụ tạo ảnh thật khi vật đặt:",
            options: [
                "Trong khoảng tiêu cự",
                "Tại tiêu điểm vật",
                "Ngoài tiêu điểm vật",
                "Bất kì vị trí nào"
            ],
            correct: 2,
            explanation: "Vật ngoài tiêu cự (d > f) → ảnh thật ngược chiều."
        },
        {
            type: "multiple",
            question: "Công thức thấu kính mỏng (quy ước dấu) là:",
            options: [
                "$$\\frac{1}{f} = \\frac{1}{d} + \\frac{1}{d'}$$",
                "$$\\frac{1}{f} = \\frac{1}{d} - \\frac{1}{d'}$$",
                "$$f = d + d'$$",
                "$$\\frac{1}{d'} = \\frac{1}{f} - \\frac{1}{d}$$"
            ],
            correct: 0,
            explanation: "1/f = 1/d + 1/d' (theo quy ước Descarte: d, d' có thể âm hoặc dương)."
        },
        {
            type: "multiple",
            question: "Vật đặt cách thấu kính hội tụ (f = 20 cm) một khoảng d = 30 cm. Ảnh cách thấu kính:",
            options: ["60 cm", "12 cm", "30 cm", "20 cm"],
            correct: 0,
            explanation: "1/d' = 1/f − 1/d = 1/20 − 1/30 = 1/60 → d' = 60 cm."
        },
        // ── LƯỢNG TỬ ÁNH SÁNG ────────────────────────────────────────────
        {
            type: "multiple",
            question: "Theo thuyết lượng tử của Planck, năng lượng của một photon có tần số f là:",
            options: ["ε = hf²", "ε = hf", "ε = h/f", "ε = hc/λ²"],
            correct: 1,
            explanation: "ε = hf = hc/λ; h = 6,626×10⁻³⁴ J.s (hằng số Planck)."
        },
        {
            type: "multiple",
            question: "Hiện tượng quang điện ngoài xảy ra khi:",
            options: [
                "Ánh sáng có bước sóng đủ dài chiếu vào kim loại",
                "Ánh sáng có tần số lớn hơn hoặc bằng tần số giới hạn f₀",
                "Cường độ ánh sáng đủ lớn",
                "Kim loại được đốt nóng đến nhiệt độ cao"
            ],
            correct: 1,
            explanation: "Điều kiện quang điện: f ≥ f₀ (tần số giới hạn), tương đương λ ≤ λ₀."
        },
        {
            type: "multiple",
            question: "Công thức Einstein về hiện tượng quang điện:",
            options: [
                "$$hf = A + \\frac{1}{2}mv_{max}^2$$",
                "$$hf = A - \\frac{1}{2}mv_{max}^2$$",
                "$$hf = \\frac{1}{2}mv_{max}^2$$",
                "$$hf = A$$"
            ],
            correct: 0,
            explanation: "hf = A + ½mv²max; năng lượng photon bằng công thoát cộng động năng ban đầu cực đại."
        },
        {
            type: "multiple",
            question: "Tia X (Rơn-ghen) có bản chất là:",
            options: [
                "Chùm electron tốc độ cao",
                "Sóng điện từ có bước sóng ngắn",
                "Chùm hạt proton",
                "Sóng âm tần số cao"
            ],
            correct: 1,
            explanation: "Tia X là sóng điện từ (λ ≈ 10⁻¹¹ – 10⁻⁸ m), có khả năng đâm xuyên mạnh."
        },
        {
            type: "multiple",
            question: "Photon ánh sáng có bước sóng λ = 0,5 μm. Năng lượng photon (h = 6,625×10⁻³⁴ J.s; c = 3×10⁸ m/s) là:",
            options: ["≈ 3,98×10⁻¹⁹ J", "≈ 2,0×10⁻¹⁹ J", "≈ 6,0×10⁻¹⁹ J", "≈ 1,0×10⁻¹⁹ J"],
            correct: 0,
            explanation: "ε = hc/λ = 6,625×10⁻³⁴×3×10⁸/(0,5×10⁻⁶) ≈ 3,975×10⁻¹⁹ J."
        },
        {
            type: "multiple",
            question: "Hiện tượng huỳnh quang là:",
            options: [
                "Vật phát quang khi bị đốt nóng",
                "Vật hấp thụ photon và phát ra photon có tần số cao hơn",
                "Vật hấp thụ ánh sáng và phát ra ánh sáng có bước sóng dài hơn",
                "Phản xạ toàn phần của ánh sáng"
            ],
            correct: 2,
            explanation: "Huỳnh quang: ánh sáng kích thích (λ ngắn, f cao) → phát quang (λ dài hơn, f thấp hơn)."
        },
        // ── VẬT LÝ HẠT NHÂN ─────────────────────────────────────────────
        {
            type: "multiple",
            question: "Hạt nhân nguyên tử $$^A_Z X$$ có:",
            options: [
                "Z proton và A neutron",
                "A proton và Z neutron",
                "Z proton và (A−Z) neutron",
                "(A−Z) proton và Z neutron"
            ],
            correct: 2,
            explanation: "Số khối A = số proton Z + số neutron N → N = A − Z."
        },
        {
            type: "multiple",
            question: "Phóng xạ β⁻ xảy ra khi:",
            options: [
                "Hạt nhân phóng ra electron và tăng số proton lên 1",
                "Hạt nhân phóng ra positron",
                "Hạt nhân phóng ra tia γ",
                "Hạt nhân phóng ra hạt α"
            ],
            correct: 0,
            explanation: "β⁻: n → p + e⁻ + ν̄; số proton tăng 1, số khối không đổi."
        },
        {
            type: "multiple",
            question: "Chu kỳ bán rã T của chất phóng xạ là:",
            options: [
                "Thời gian để toàn bộ chất phóng xạ phân rã",
                "Thời gian để một nửa số hạt nhân ban đầu phân rã",
                "Thời gian để chất phóng xạ mất hết hoạt độ",
                "Thời gian phóng xạ tỉ lệ với nhiệt độ"
            ],
            correct: 1,
            explanation: "Sau mỗi chu kỳ T, số hạt nhân còn lại giảm đi một nửa: N = N₀/2."
        },
        {
            type: "multiple",
            question: "Năng lượng liên kết hạt nhân được tính theo công thức Einstein:",
            options: [
                "$$W = \\Delta m . c$$",
                "$$W = \\Delta m . c^2$$",
                "$$W = \\Delta m / c^2$$",
                "$$W = \\Delta m . c^3$$"
            ],
            correct: 1,
            explanation: "Năng lượng liên kết W = Δm.c², Δm là độ hụt khối."
        },
        {
            type: "multiple",
            question: "Phản ứng phân hạch là:",
            options: [
                "Hai hạt nhân nhẹ kết hợp tạo hạt nhân nặng hơn",
                "Hạt nhân nặng bị vỡ thành hai hạt nhân nhẹ hơn",
                "Hạt nhân phóng ra tia α",
                "Electron kết hợp với proton"
            ],
            correct: 1,
            explanation: "Phân hạch: hạt nhân nặng (U-235, Pu-239...) bị vỡ khi hấp thụ neutron chậm, giải phóng năng lượng lớn."
        },
        {
            type: "multiple",
            question: "Hằng số phóng xạ λ liên hệ với chu kỳ bán rã T theo công thức:",
            options: [
                "λ = T/ln2",
                "λ = ln2/T",
                "λ = T.ln2",
                "λ = 1/T"
            ],
            correct: 1,
            explanation: "λ = ln2/T = 0,693/T."
        },
        {
            type: "multiple",
            question: "Chất phóng xạ có T = 8 ngày. Sau 24 ngày, lượng còn lại so với ban đầu là:",
            options: ["1/2", "1/4", "1/8", "1/16"],
            correct: 2,
            explanation: "n = 24/8 = 3 chu kỳ. N = N₀×(1/2)³ = N₀/8."
        },
        {
            type: "multiple",
            question: "Trong phản ứng nhiệt hạch, điều kiện cần thiết là:",
            options: [
                "Nhiệt độ rất thấp (gần 0 K)",
                "Nhiệt độ rất cao (~10⁷ – 10⁸ K) để thắng lực đẩy Coulomb",
                "Áp suất bình thường",
                "Hạt nhân phải có số khối lớn"
            ],
            correct: 1,
            explanation: "Nhiệt hạch cần nhiệt độ hàng chục triệu độ để các hạt nhân nhẹ đủ năng lượng vượt hàng rào Coulomb."
        },
        {
            type: "multiple",
            question: "Tia phóng xạ nào không bị lệch trong điện trường?",
            options: ["Tia α", "Tia β⁻", "Tia β⁺", "Tia γ"],
            correct: 3,
            explanation: "Tia γ là sóng điện từ (không mang điện) → không bị lệch trong điện/từ trường."
        },
        {
            type: "multiple",
            question: "Độ hụt khối Δm của hạt nhân $$^A_Z X$$ được tính bằng:",
            options: [
                "Δm = Z.m_p + (A−Z).m_n − m_X",
                "Δm = m_X − Z.m_p − (A−Z).m_n",
                "Δm = A.m_p − m_X",
                "Δm = Z.m_n + (A−Z).m_p − m_X"
            ],
            correct: 0,
            explanation: "Δm = Z.mₚ + N.mₙ − mₓ; luôn dương vì hạt nhân bền hơn tập hợp các nuclon riêng lẻ."
        },
        {
            type: "multiple",
            question: "Phản ứng hạt nhân nào dưới đây là phản ứng nhiệt hạch?",
            options: [
                "$$^{235}_{92}U + n \\to ^{141}_{56}Ba + ^{92}_{36}Kr + 3n$$",
                "$$^2_1H + ^3_1H \\to ^4_2He + n$$",
                "$$^{226}_{88}Ra \\to ^{222}_{86}Rn + ^4_2He$$",
                "$$^{14}_7N + ^4_2He \\to ^{17}_8O + ^1_1H$$"
            ],
            correct: 1,
            explanation: "D + T → He-4 + n là phản ứng nhiệt hạch (tổng hợp hạt nhân nhẹ), giải phóng ~17,6 MeV."
        },
        {
            type: "multiple",
            question: "Năng lượng liên kết riêng (năng lượng liên kết trên mỗi nuclon) lớn nhất ứng với hạt nhân:",
            options: [
                "Hydro (H)",
                "Urani (U)",
                "Sắt (Fe, A ≈ 56)",
                "Heli (He)"
            ],
            correct: 2,
            explanation: "Hạt nhân Fe-56 có năng lượng liên kết riêng lớn nhất (~8,8 MeV/nuclon), bền vững nhất."
        },
        // ── TỰ LUẬN ──────────────────────────────────────────────────────
        {
            type: "essay",
            question: "Mạch dao động LC có L = 1 mH và C = 25 pF.\n\na) Tính tần số dao động riêng.\nb) Điện tích cực đại trên tụ Q₀ = 5×10⁻⁸ C. Tính năng lượng điện trường cực đại và năng lượng toàn phần của mạch.",
            suggested: "a) f = 1/(2π√LC) = 1/(2π√(10⁻³×25×10⁻¹²))\n= 1/(2π×5×10⁻⁸) ≈ 3,18×10⁶ Hz ≈ 3,18 MHz.\n\nb) W_điện cực đại = Q₀²/(2C) = (5×10⁻⁸)²/(2×25×10⁻¹²)\n= 25×10⁻¹⁶/(50×10⁻¹²) = 5×10⁻⁵ J.\nNăng lượng toàn phần = W_điện cực đại = 5×10⁻⁵ J (vì lý tưởng, bảo toàn).",
            points: 25,
            explanation: "AI chấm: f ≈ 3,18 MHz; W = Q₀²/(2C) = 5×10⁻⁵ J."
        },
        {
            type: "essay",
            question: "Ánh sáng chiếu từ thủy tinh (n₁ = 1,5) sang không khí (n₂ = 1).\n\na) Tính góc giới hạn phản xạ toàn phần.\nb) Tia sáng trong thủy tinh tới mặt phân cách với góc tới θ = 45°, xảy ra hiện tượng gì? Giải thích.",
            suggested: "a) sinθ_gh = n₂/n₁ = 1/1,5 = 2/3\nθ_gh = arcsin(2/3) ≈ 41,8°.\n\nb) θ = 45° > θ_gh ≈ 41,8° → xảy ra phản xạ toàn phần.\nÁnh sáng phản xạ hoàn toàn trở lại thủy tinh, không có tia khúc xạ sang không khí.",
            points: 25,
            explanation: "AI chấm: θ_gh ≈ 41,8°; θ = 45° > θ_gh → phản xạ toàn phần."
        },
        {
            type: "essay",
            question: "Hạt nhân $$^{210}_{84}Po$$ phóng xạ α, tạo thành hạt nhân con X và hạt α $$^4_2He$$.\n\na) Xác định hạt nhân X (số proton Z_X và số khối A_X).\nb) Hỏi sau 3 chu kỳ bán rã (T = 138 ngày), khối lượng Po-210 còn lại là bao nhiêu % so với ban đầu?\nc) Nếu m₀ = 1 g, tính khối lượng Po còn lại sau 414 ngày.",
            suggested: "a) Bảo toàn số khối: 210 = 4 + A_X → A_X = 206.\nBảo toàn điện tích: 84 = 2 + Z_X → Z_X = 82.\nHạt nhân X là $$^{206}_{82}Pb$$ (chì-206).\n\nb) n = 3T/T = 3 chu kỳ.\nN/N₀ = (1/2)³ = 1/8 = 12,5%.\n\nc) m = m₀ × (1/2)³ = 1 × 1/8 = 0,125 g.",
            points: 25,
            explanation: "AI chấm: X là Pb-206; còn 12,5%; m = 0,125 g sau 414 ngày."
        }
    ]
};
